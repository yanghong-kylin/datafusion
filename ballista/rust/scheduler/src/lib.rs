...
impl SchedulerGrpc for SchedulerServer {
        ...
        } = request.into_inner()
        {
            ...
            let task: Result<Option<_>, Status> = if can_accept_task {
                let plan = match self.policy {
                    TaskSchedulingPolicy::PullStaged => self
                        .state
                    ...
                    TaskSchedulingPolicy::PullAllAtOnce => self
                        .state
    ...
    async fn execute_query(
        ...
        } = request.into_inner()
        {
            ...
            let (push_based_shuffle, tx_job) = match self.policy {
                TaskSchedulingPolicy::PullStaged => (false, None),
                TaskSchedulingPolicy::PullAllAtOnce => (true, None),
            ...
            tokio::spawn(async move {
                ...
                let stages = fail_job!(planner
                    .plan_query_stages(&job_id_spawn, plan, push_based_shuffle)
                    .await
                    .map_err(|e| {
